 ...
 
{
	...
	/* openconnection with the X server. */
	...
	/* make the new window occupy 1/9 of the screen's size. */
    	// Load image
    FILE *file = fopen ("/home/zhenwei-qian/Pictures/0.png", "r");
    if (!file)
        return 1;
    ...
    XImage *ximage = XCreateImage (display, DefaultVisual (display, DefaultScreen (display)), 
        DefaultDepth(display, DefaultScreen (display)), ZPixmap, 0, (char*)data, width_, height_, 8, rowbytes);
    if (ximage) {
